...
#else
    localSphere = (sphere*)&f->gme_geometry()->gme_equation_for_update();            // 获取待处理的Sphere
    SPAinterval u_interval = f->gme_geometry()->gme_equation().gme_param_range_u();  // 参数域u上的范围
    SPAinterval v_interval = f->gme_geometry()->gme_equation().gme_param_range_v();  // 参数域v上的范围
#endif
...
#else
    if(f->gme_geometry()->gme_equation().gme_closed_u()) u_tolerance = localSphere->gme_param_range_u().gme_length() / 2;  // 判断跨边界所用值
    if(f->gme_geometry()->gme_equation().gme_closed_v()) v_tolerance = localSphere->gme_param_range_v().gme_length() / 2;
#endif
...
bool sphere_faceter::getLoops() {
    ...
    for(LOOP* lp = f->gme_loop(); lp; lp = lp->gme_next()) {
        std::vector<std::tuple<int, point, point>> cross_pairs;  // 跨越边界的点对
        std::vector<CDT::V2d<double>> tempPS;                    // 离散的点集
        loop_type lptype;                                        // loop类型
        gme_api_loop_type(lp, lptype);                           // 获取loop的类型
#endif
        unsigned ic = 0;
...
#else
            gme_sg_add_pcurve_to_coedge(coedge);  // 给coedge添加参数曲线
            PCURVE* PC = coedge->geometry();      // 获得coedge的参数曲线
            // 该边为null edge
            if(PC == nullptr) {  // 当这条边退化成一个null边，也就是一个点的时候。
                // 在Sphere中，只有奇点会出现这种情况，也就是参数u=PI/2或u=-PI/2的时候
                param = f->gme_geometry()->gme_equation().gme_param(polyline[0]);  // 从三维空间获得这个点的参数域位置
                                                                                   // 由于奇点从三维空间到二维参数域无法正确表示成一条边，而是会成为一个点。所以需要把这个点还原成一条边。
                                                                                   // 通过在参数u边界上的两个点来表示这条边。
#endif
...
#else
                pcurve pc = PC->gme_equation();  // PCRUVE类的pcruve
                for(int i = 0; i < nP; i++) {
                    param = pc.gme_eval_position(polyline[i], params[i], 1);                       // 获得第i个参数域上的点
                    SPApar_pos param1 = f->gme_geometry()->gme_equation().gme_param(polyline[i]);  // 获得从三维映射到参数域上的点
#endif
...
#else
                    // 如果这个点在跨周期的时候，有可能会落在参数域的左边界上，也有可能会落在参数域的右边界上。所以需要进行一些修正
                    if(f->gme_geometry()->gme_equation().gme_closed_u() && equal(param1.u, u_le) && equal(param.u, u_ri)) {  // 如果param1计算为左边界，param计算为右边界则以param为准，其余情况以param1为准
                        param1.u = u_ri;
                    }
                    if(f->gme_geometry()->gme_equation().gme_closed_v() && equal(param1.v, v_le) && equal(param.v, v_ri)) {  // 如果param1计算为左边界，param计算为右边界则以param为准，其余情况以param1为准
#endif
                        param1.v = v_ri;
                    }
...
#else
        SPApar_pos upp_pp = localSphere->gme_param(upper_singularity);                          // 上方奇点的参数域点
        SPAposition lower_singularity = sphere_centre - sphere_radius * sphere_pole_direction;  // 球面下方奇点
        SPApar_pos lpp_pp = localSphere->gme_param(lower_singularity);                          // 下方奇点的参数域点
#endif
...
#else
                        SPAposition following_pt = f->gme_geometry()->gme_equation().gme_eval_position(SPApar_pos(tempPS[j].x, tempPS[j].y));  // 从tempPS[j]的参数域点获得映射的三维空间点
#endif
...
#else
                        SPAposition following_point = f->gme_geometry()->gme_equation().gme_eval_position(SPApar_pos(tempPS[j].x, tempPS[j].y));
#endif
...
#else
    bool closed_u = f->gme_geometry()->gme_equation().gme_closed_u();  // 在u上是否封闭
    bool closed_v = f->gme_geometry()->gme_equation().gme_closed_v();  // 在v上是否封闭
#endif
