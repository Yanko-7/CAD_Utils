...
void _print_curve(curve* c) {
    SPAinterval c_range = c->gme_param_range();
    printf("period %lf\n", c->gme_param_period());
...
void _print_edge(EDGE* e) {
    printf("closed %d periodic %d\n", e->gme_closed(), e->gme_periodic());
    SPAinterval p_range = e->gme_param_range();
    printf("start %lf end %lf\n", p_range.start_pt(), p_range.end_pt());
    printf("startp %lf endp %lf\n", e->gme_start_param(), e->gme_end_param());
}
void _print_entity(ENTITY* ent) {
    BODY* body = (BODY*)ent;
    LUMP* lmp = body->gme_lump();
    for(int ilmp = 0; lmp != nullptr; lmp = lmp->gme_next(), ilmp++) {
        printf("LUMP %d\n", ilmp);
        SHELL* shl = lmp->gme_shell();
        for(int is = 0; shl != nullptr; shl = shl->gme_next(), is++) {
            printf(" SHELL %d\n", is);
            FACE* fc = shl->gme_face();
            for(int ifc = 0; fc != nullptr; fc = fc->gme_next(), ifc++) {
                printf("  FACE %d\n", ifc);
                std::cout << "Face Type? (1: Plane; 2: Cone; 3: Sphere; 4: Torus; 10: Spline): " << fc->gme_geometry()->gme_equation().gme_type() << std::endl;

                SPAinterval u_interval = fc->gme_geometry()->gme_equation().gme_param_range_u();
                SPAinterval v_interval = fc->gme_geometry()->gme_equation().gme_param_range_v();
                ...
                if(fc->gme_geometry()->gme_equation().gme_type() == 11 || fc->gme_geometry()->gme_equation().gme_type() == 4) {
                    // SPAunit_vector nor = fc->geometry()->equation().eval_normal()

                    curve* u_le = fc->gme_geometry()->gme_equation().gme_v_param_line(u_interval.end_pt());
                    SPAinterval interval = u_le->gme_param_range();
                    // std::cout << interval.start_pt() << " " << interval.end_pt() << std::endl;
                    std::cout << u_interval.end_pt() << "\t" << u_le->gme_length(interval.start_pt(), interval.end_pt()) << std::endl;

                    u_le = fc->gme_geometry()->gme_equation().gme_v_param_line(0.99999999);
                    interval = u_le->gme_param_range();
                    // std::cout << interval.start_pt() << " " << interval.end_pt() << std::endl;
                    std::cout << "0.99999999"
                              << "\t" << u_le->gme_length(interval.start_pt(), interval.end_pt()) << std::endl;

                    u_le = fc->gme_geometry()->gme_equation().gme_v_param_line(0.9999);
                    interval = u_le->gme_param_range();
                    // std::cout << interval.start_pt() << " " << interval.end_pt() << std::endl;
                    std::cout << 0.9999 << "\t" << u_le->gme_length(interval.start_pt(), interval.end_pt()) << std::endl;

                    SPAunit_vector normal = fc->gme_geometry()->gme_equation().gme_eval_normal(SPApar_pos(u_interval.end_pt(), v_interval.start_pt()));
                    SPAunit_vector outdir = fc->gme_geometry()->gme_equation().gme_eval_outdir(SPApar_pos(u_interval.end_pt(), v_interval.start_pt()));
                ...
                printf("\tclosed_u: %d\t closed_v: %d\n", fc->gme_geometry()->gme_equation().gme_closed_u(), fc->gme_geometry()->gme_equation().gme_closed_v());
                LOOP* lp = fc->gme_loop();
                for(int ilp = 0; lp != nullptr; lp = lp->gme_next(), ilp++) {
                    loop_type lptype;
                    ...
                    for(int ic = 0; cedge != fcedge || ic == 0; cedge = cedge->next(), ++ic) {
                        ...
                        SPAposition point = cedge->gme_start_pos();
                        printf("\tstart_point: (%.2f, %.2f, %.2f)\t%d\n", point.x(), point.y(), point.z(), cedge->gme_starts_at_singularity());
                        point = cedge->gme_end_pos();
                        printf("\tend_point: (%.2f, %.2f, %.2f)\t%d\n", point.x(), point.y(), point.z(), cedge->gme_ends_at_singularity());
                        int _typeid = fc->gme_geometry()->gme_equation().gme_type();
                        if(_typeid == 4 || _typeid == 10) {
                            int nP = 0;
                            SPAposition* polyline;
                            double* params;
                            get_facet_edge_points_and_params1(cedge->edge(), polyline, params, nP);
                            torus* tori = (torus*)&fc->gme_geometry()->gme_equation_for_update();
                            printf("reverse-v: %d\n", tori->reverse_v);
                            gme_sg_add_pcurve_to_coedge(cedge);
                            PCURVE* PC = cedge->geometry();
                            if(PC == nullptr) continue;
                            pcurve pc = PC->gme_equation();
                            //
                            if(!r) {  // 基于指向进行不同处理
                                for(int i = 0; i < nP; i++) {
                                    SPApar_pos param1 = pc.gme_eval_position(polyline[i], params[i], 1);
                                    SPApar_pos param = fc->gme_geometry()->gme_equation().gme_param(polyline[i]);
                                    SPAvector normal = fc->gme_geometry()->gme_equation().gme_eval_normal(param);
...
void _print_mesh_result(ENTITY* ent) {
    ...
    printf("\t=====EDGE's num: %d=====\n", eg_list.gme_count());
    EDGE* e = (EDGE*)eg_list.gme_first();
    for(int i_e = 0; i_e < eg_list.gme_count(); i_e++) {
        if(e->gme_length() != 0) printf("===EDGE[%d]: \033[47;30m%s\033[0m\n", i_e, e->geometry()->gme_equation().gme_type_name());
        SPAposition* pos = nullptr;
        int n;
        gme_api_get_facet_edge_points(e, pos, n);
        ...
        e = (EDGE*)eg_list.gme_next();
    }

    ENTITY_LIST f_list;
    get_faces(ent, f_list);
    printf("\t=====FACE's num: %d=====\n", f_list.gme_count());
    FACE* f = (FACE*)f_list.gme_first();
    INDEXED_MESH* mesh;
    for(int i_f = 0; i_f < f_list.gme_count(); i_f++, f = (FACE*)f_list.gme_next()) {
        printf("===FACE[%d]: \033[47;30m%s\033[0m\n", i_f, f->gme_geometry()->gme_equation().gme_type_name());
...
void FaceterEvaluator::FacetEntity() {
    ...
    for(int i = 0; i < face_list.count(); i++, face = dynamic_cast<FACE*>(face_list.next())) {
        ...
        if(use_gme_) {
            ...
            face_error_map_gme_.emplace(face, FacetInfo());
            face_error_map_gme_.at(face).time = time_gme.count();
            face_mesh_map_gme_.emplace(face, idx_mesh);
        }
    }
}
...
void FaceterEvaluator::ComputeAllError() {
    for(auto it: face_mesh_map_) {
        ...
        if(use_gme_) ComputeErrorBySample(it.first, 1);
        face_hausdorff_map_.emplace(it.first, HausdorffDistance(face_mesh_map_.at(it.first), face_mesh_map_gme_.at(it.first)));
    }
}
...
void FaceterEvaluator::ComputeError(FACE* face, int engine) {
    ...
    if(engine == 1) {
        int polygon_nb = face_mesh_map_gme_.at(face)->get_num_polygon();
        face_error_map_gme_.at(face).triangle_number = polygon_nb;
        face_error_map_gme_.at(face).vertex_number = face_mesh_map_gme_.at(face)->get_num_vertex();

        face_mesh_map_gme_.at(face)->set_par_pos_mapping_01(false);
        for(int i = 0; i < polygon_nb; i++) {
            int vertex_nb = face_mesh_map_gme_.at(face)->get_polygon(i)->num_vertex();
            ...
            for(int j = 0; j < vertex_nb; j++) {
                pos_3d[j] = face_mesh_map_gme_.at(face)->get_polygon(i)->get_vertex(j)->get_position() * transf_;
                ...
                pos_2d[j] = face_mesh_map_gme_.at(face)->get_polygon(i)->get_vertex(j)->get_uv();
            ...
            face_error_map_gme_.at(face).max_error = std::max(face_error_map_gme_.at(face).max_error, error);
            face_error_map_gme_.at(face).min_error = std::min(face_error_map_gme_.at(face).min_error, error);
            SPApar_vec v01(pos_2d[1] - pos_2d[0]), v02(pos_2d[2] - pos_2d[0]);
            // 以三角面片参数域上的面积作为权重
            double tmp = fabs(v01 * v02) / 2;
            face_error_map_gme_.at(face).total_error += fabs(v01 * v02) / 2 * error;
        }
    }
}
...
void FaceterEvaluator::ComputeErrorBySample(FACE* face, int engine) {
    ...
    for(int i = 0; i < u_sample; i++) {
        for(int j = 0; j < v_sample; j++) {
            ...
            if(pfc == point_face_containment::point_inside_face) {
                ...
                if(engine == 1) {
                    error = PointToMeshDistance(p, face_mesh_map_gme_.at(face));
                    face_error_map_gme_.at(face).max_error = std::max(face_error_map_gme_.at(face).max_error, error);
                    face_error_map_gme_.at(face).min_error = std::min(face_error_map_gme_.at(face).min_error, error);
                }
            }
    ...
    if(engine == 1) {
        face_error_map_gme_.at(face).triangle_number = face_mesh_map_gme_.at(face)->get_num_polygon();
        face_error_map_gme_.at(face).vertex_number = face_mesh_map_gme_.at(face)->get_num_vertex();
    }
}
...
static void get_polylines_from_faceted_edges(ENTITY_LIST& edges, std::vector<FacetMesh::EdgeData>& edges_data, bool use_acis) {
    ...
    for(ENTITY* ent = edges.first(); ent; ent = edges.next()) {
        ...
        if(use_acis)
            ...
            out = gme_api_get_facet_edge_points(edge, pos, nP);
        if(!out.ok()) {
            ACIS_DELETE[] pos;
            continue;
        }
...
static void gme_get_triangles_from_faceted_faces_withUV(FACE* face, FacetMesh::FaceData& face_data) {
    ...
    face_data.coords.resize(3 * nv);
    mesh->serialize_positions("gme", face_data.coords.data());

    bool const has_normals = mesh->has_normals();
    if(has_normals) {
        face_data.normal_coords.resize(3 * nv);

        mesh->serialize_normals("gme", face_data.normal_coords.data());
    }
    ...
    face_data.triangles.resize(3 * ntri);
    int ntri_actual = mesh->serialize_triangles("gme", face_data.triangles.data());
    while(ntri_actual < ntri) {
        face_data.triangles.pop_back();
        ntri_actual = static_cast<int>(face_data.triangles.size());
    }
...
void WriteMeshInObjFormat(const std::string& filename, ENTITY_LIST& faces, bool use_acis) {
    ...
    for(ENTITY* ent = faces.first(); ent; ent = faces.next()) {
        ...
        if(use_acis)
            ...
            gme_get_triangles_from_faceted_faces_withUV(face, face_data);
