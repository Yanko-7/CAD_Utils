...
#    else
    localCone = (cone*)&f->gme_geometry()->gme_equation_for_update();
    // 获取锥面在u上的范围
...
#    else
            if(localCone->gme_singular_u(u_le)) {
#    endif
                singular_ule = true;
            }
            break;
...
#    else
            if(localCone->gme_singular_u(u_ri)) {
#    endif
                singular_uri = true;
            }
            break;
...
#    else
    if(f->gme_geometry()->gme_equation().gme_closed_u()) u_tolerance = localCone->gme_param_range_u().gme_length() / 2;  // 判断跨边界所用值
    if(f->gme_geometry()->gme_equation().gme_closed_v()) v_tolerance = localCone->gme_param_range_v().gme_length() / 2;
#    endif
...
#    else
            double radius = localCone->base.gme_GetMajorAxisLength();  // 圆锥的半径长度
#    endif
...
#    ifdef FACETER_USE_ACIS
            double radius = localCone->base.GetMajorAxisLength();
#    else
            double radius = localCone->base.gme_GetMajorAxisLength();  // 圆锥面的半径
#    endif
            double seg_angle = GetSegAngle(this->st / 2.0, this->nt / 2.0, radius);  // 分段最小的角度值
...
#    else
    for(LOOP* lp = f->gme_loop(); lp; lp = lp->gme_next(), j++) {
#    endif
...
#    else
                param = f->gme_geometry()->gme_equation().gme_param(polyline[i]);
                tempps.push_back(point(param.u, param.v));
            }
            FaceterFaceInfo face_info(u_ri, u_le, v_ri, v_le, u_tolerance, v_tolerance, f->gme_geometry()->gme_equation().gme_closed_u(), f->gme_geometry()->gme_equation().gme_closed_v());
#    endif
...
#    ifdef FACETER_USE_ACIS
    FaceterFaceInfo face_info(u_ri, u_le, v_ri, v_le, u_tolerance, v_tolerance, f->geometry()->equation().closed_u(), f->geometry()->equation().closed_v());
#    else
    FaceterFaceInfo face_info(u_ri, u_le, v_ri, v_le, u_tolerance, v_tolerance, f->gme_geometry()->gme_equation().gme_closed_u(), f->gme_geometry()->gme_equation().gme_closed_v());  // 整个面的信息
#    endif
...
void cone_faceter::attachMesh() {
    ...
    // INDEXED_MESH* mesh = new INDEXED_MESH("gme", my_vertices.size(), my_triangles.size(), my_triangles.size() * 3);
    // for(int i = 0; i < my_vertices.size(); i++) {
...
#    else
        ...
        SPAunit_vector normal = f->gme_geometry()->gme_equation().gme_eval_outdir(param);
#    endif
        if(r) normal = -normal;
        mesh->add_vertex("gme", pos, normal, param);
    }
...
#    ifdef FACETER_USE_ACIS
            SPAposition apex = localCone->get_apex();
#    else
            SPAposition apex = localCone->gme_get_apex();
#    endif
            double sign = 1.0;
...
#    ifdef FACETER_USE_ACIS
                SPAunit_vector tmp_n = f->geometry()->equation().eval_normal(tmp_p);
#    else
                SPAunit_vector tmp_n = f->gme_geometry()->gme_equation().gme_eval_normal(tmp_p);
#    endif
                if(f->sense()) tmp_n = -tmp_n;
                // printf("%lf %lf\t%lf %lf %lf\n", tmp_p.u, tmp_p.v, tmp_n.x(), tmp_n.y(), tmp_n.z());
                // mesh->get_vertex(p1).set_uv(tmp_p);
