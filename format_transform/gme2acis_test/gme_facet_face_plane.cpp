...
#else
    for(LOOP* lp = f->gme_loop(); lp != nullptr; lp = lp->gme_next()) {
#endif
...
// 在调试模式下输出环的编号
        ...
        for(int ic = 0; cedge != fcedge || ic == 0; cedge = cedge->next(), ic = 1) {
            ...
            if(eg->geometry() == nullptr) {
                ...
                // 离散边获取点存入pl1
                gme_api_get_facet_edge_points(eg, pl1, n_sample1);
...
#else
                points.push_back({f->gme_geometry()->gme_equation().gme_param(pos).u, f->gme_geometry()->gme_equation().gme_param(pos).v});
#endif
...
// 在调试模式下打印三角剖分后的顶点个三角形
    ...
    // "gme"是网格名称
    INDEXED_MESH* mesh = new INDEXED_MESH("gme", points.size(), cdt.triangles.size(), cdt.triangles.size() * 3);
    // 遍历所有点
...
#else
        SPAposition pos = f->gme_geometry()->gme_equation().gme_eval_position(par_pos);
        // 计算参数空间点位置par_pos对应的法向量
        SPAunit_vector uv = f->gme_geometry()->gme_equation().gme_eval_normal(par_pos);
#endif
        if(r) {
            uv = -uv;
        }
        mesh->add_vertex("gme", pos, uv, par_pos);
    }
    ...
    for(const auto& t: cdt.triangles) {
        // 向mesh对象中添加一个新的多边形
        mesh->add_polygon("gme", m, 3);
        // 获取新添加的多边形对象poly0
        indexed_polygon* poly0 = mesh->get_polygon("gme", m);
        // 将三角形的顶点分别设置为多边形的顶点
        poly0->set_vertex("gme", 0, &mesh->get_vertex("gme", t.vertices[0]));
        poly0->set_vertex("gme", 1, &mesh->get_vertex("gme", t.vertices[1]));
        poly0->set_vertex("gme", 2, &mesh->get_vertex("gme", t.vertices[2]));
