...
#else
    SPAposition a3 = spl->gme_eval_position(SPApar_pos(a.x, a.y));
    SPAposition b3 = spl->gme_eval_position(SPApar_pos(b.x, b.y));
    SPApar_pos a2(a.x, a.y);
    SPAvector ab3 = b3 - a3;
    x SPAunit_vector an = spl->gme_eval_normal(a2);
    if(f->sense()) an = -an;
    SPAvector dir3 = an * ab3;
    SPAunit_vector udir3(dir3.x(), dir3.y(), dir3.z());
    return spl->gme_param_dir(udir3, a2);
#endif
}
...
void spline_faceter::init() {
    ...
    SPAinterval v_interval = f->geometry()->equation().param_range_v();
#else
    spl = (spline*)&f->gme_geometry()->gme_equation();  // 获取surface
    SPApar_box face_par_box = spl->param_range();       // 获取主参数域边界框
    SPAinterval u_interval = spl->gme_param_range_u();  // 获取u方向参数范围
    SPAinterval v_interval = spl->gme_param_range_v();  // 获取v方向参数范围
#endif
...
#else
    if(f->gme_geometry()->gme_equation().gme_closed_u()) {
        u_tolerance = spl->gme_param_range_u().gme_length() / 2;  // 判断跨边界所用值
        if(spl->gme_singular_u(v_le)) {
            singular_vle = true;
        } else {  // B_7  B14的样例需要通过下列判断才能确定奇点情况
            curve* vle = spl->gme_v_param_line(v_le);
            SPAinterval interval = vle->gme_param_range();
            if(is_equal(vle->gme_length(interval.start_pt(), interval.end_pt()), 0.0)) {
                singular_vle = true;
            }
        }
        if(spl->gme_singular_u(v_ri)) {
            singular_vri = true;
        } else {
            curve* vri = spl->gme_v_param_line(v_ri);
            SPAinterval interval = vri->gme_param_range();
            if(is_equal(vri->gme_length(interval.start_pt(), interval.end_pt()), 0.0)) {
#endif
                singular_vri = true;
            }
        }
...
#else
    if(f->gme_geometry()->gme_equation().gme_closed_v()) {
        v_tolerance = spl->gme_param_range_v().gme_length() / 2;

        if(spl->gme_singular_u(u_le)) {
            singular_ule = true;
        } else {  // B_7  B14的样例需要通过下列判断才能确定奇点情况
            curve* ule = spl->gme_v_param_line(u_le);
            SPAinterval interval = ule->gme_param_range();
            if(is_equal(ule->gme_length(interval.start_pt(), interval.end_pt()), 0.0)) {
                singular_ule = true;
            }
        }
        if(spl->gme_singular_u(u_ri)) {
            singular_uri = true;
        } else {
            curve* uri = spl->gme_v_param_line(u_ri);
            SPAinterval interval = uri->gme_param_range();
            if(is_equal(uri->gme_length(interval.start_pt(), interval.end_pt()), 0.0)) {
#endif
                singular_uri = true;
            }
        }
...
#else
    if((singular_ule || singular_uri) && f->gme_geometry()->gme_equation().gme_closed_v() || (singular_vle || singular_vri) && f->gme_geometry()->gme_equation().gme_closed_u()) {
#endif
...
void spline_faceter::attachMeshByQuad(QuadTree& tree) {
    ...
    // 创建索引网格对象
    INDEXED_MESH* mesh = new INDEXED_MESH("gme", vertice.size(), triangles.size(), triangles.size() * 3);
    // 遍历所有顶点，添加到网格中
...
void spline_faceter::attachMesh() {
...
#else
    surface* surf = &f->gme_geometry()->gme_equation_for_update();
#endif
    ...
    for(double v = v_le; v < v_ri + SPAresabs / 2; v += dv) {
        ...
        for(unsigned i = 0; i < edges.size(); i++) {
...
#else
            straight edge_line(b, gme_normalise(edge_dir));
            SPAinterval edge_range(0.0, (edge_dir).len());
            edge_line.gme_limit(edge_range);
            ...
            if(results != NULL) {
                ...
                double p1 = results->param1;
                SPAposition inters = v_line.gme_eval_position(p1);
#endif
                unsigned j = 0;
    ...
    INDEXED_MESH* mesh = new INDEXED_MESH("gme", vertice.size(), pcdt->triangles.size(), pcdt->triangles.size() * 3);
    for(int i = 0; i < vertice.size(); i++) {
        CDT::V2d<double> pp = vertice[i];
        SPApar_pos param(pp.x, pp.y);
#ifdef FACETER_USE_ACIS
        ...
        SPAunit_vector normal = f->geometry()->equation().eval_normal(param);
#else
        SPAposition pos = f->gme_geometry()->gme_equation().gme_eval_position(param);
        // SPAposition pos = SPAposition(pp.x * 10.0, pp.y * 10.0, 0.0f);
        SPAunit_vector normal = f->gme_geometry()->gme_equation().gme_eval_normal(param);
#endif
        if(f->sense()) normal = -normal;
        mesh->add_vertex("gme", pos, normal, param);
    }
...
#else
    SPApar_pos param1 = {u, vrange.gme_mid_pt()};
    SPApar_pos param2 = {u, vrange.start_pt()};

    SPAposition pos1 = f->gme_geometry()->gme_equation().gme_eval_position(param1);
    SPAposition pos2 = f->gme_geometry()->gme_equation().gme_eval_position(param2);
#endif
    if(pos1 == pos2) {
        return true;
    }
...
bool spline_faceter::getLoops() {
    ...
    for(LOOP* lp = f->loop(); lp; lp = lp->next()) {
        ...
        for(COEDGE* coedge = lp->start(); coedge != lp->start() || ic == 0; coedge = coedge->next(), ic++) {
            ...
            gme_sg_add_pcurve_to_coedge(coedge);
            PCURVE* PC = coedge->geometry();
            if(PC == nullptr) {  // 拿不到PCURVE表达式，直接代FACE，用三维点反求二维参数域点
...
#else
                a3 = spl->gme_eval_position(SPApar_pos(a.x, a.y));
                b3 = spl->gme_eval_position(SPApar_pos(b.x, b.y));
#endif
...
#else
            SPAunit_vector an = spl->gme_eval_normal(SPApar_pos(a.x, a.y));
            if(f->sense()) an = -an;
            SPAvector dir3 = an * ab3;
            SPAunit_vector udir3(dir3.x(), dir3.y(), dir3.z());
            SPApar_dir dir = spl->gme_param_dir(udir3, a2);
#endif
